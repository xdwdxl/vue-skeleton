...
export class I18nManager implements I18nInstance {
  ...
  public async loadMeta(): Promise<void> {
    ...
    bus.publish('i18n.meta.updated', { available, default: def, timezone: undefined, currency: undefined })
  }
  ...
  private normalizeLocale(locale: string | undefined | null): string {
    ...
    const fallback = supportedSet.has(this.options.fallbackLocale) ? this.options.fallbackLocale : (supported[0] || DEFAULT_OPTIONS.fallbackLocale)
  ...
  private resolveMessage(bundle: I18nMessages | undefined, key: string): I18nMessageTree | undefined {
  ...
  private format(msg: I18nMessageTree, params?: I18nParams): string {
    ...
    const fmt = typeof msg === 'object' ? (msg as { fmt?: 'plain' | 'icu' }).fmt ?? 'plain' : 'plain'
  ...
  private handleOpeningBrace(text: string, startIndex: number, params: I18nParams): { segment: string; nextIndex: number } {
  ...
  private serializeValue(val: unknown): string {
    if (typeof val === 'object' && val !== null) {
      const v = val as any
      if (typeof v.toString === 'function' && v.toString !== Object.prototype.toString && !Array.isArray(v)) {
        return String(v)
      }
      try {
  ...
  private processPluralOrSelect(key: string, type: 'plural' | 'select', body: string, params: I18nParams): string {
  ...
  private bindBus() {
    ...
    bus.subscribe('i18n.updated', (msg) => {
      ...
      if (locale && app === this.options.appId) {
        ...
        API.GetConfig({ dataId, group: this.options.group }).then((res) => {
          if (res?.data) {
